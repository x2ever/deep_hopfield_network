...
class DeepHopfield:
    ...
    def forward(self, x, y):
        ...
        self.hopfield_loss = mse_loss_fn(
            bi_latent_vectors.detach(), my_sign(self.hopfield.label_latent_vectors[y])
        )
        self.label_loss = \
...
if __name__ == "__main__":
    ...
    for step in range(total_epoch + 1):
        ...
        for b, (data, target) in enumerate(train_loader):
            ...
            hopfield_loss = (total_epoch - step) / total_epoc   h * model.hopfield_loss
            label_loss = step / total_epoch * model.label_loss
            restoring_loss = (total_epoch - step) / total_epoch * model.restoring_loss
        ...
        with torch.no_grad():
            ...
            for b, (data, target) in enumerate(test_loader):
                real_labels.extend(target.numpy().tolist())
                ...
                predict_labels.extend(clustered_labels.cpu().numpy().tolist())
                latent_vectors = model.encoder(data).cpu().numpy().tolist()

                coordinates.extend(latent_vectors)

            coordinates.extend(model.hopfield.label_latent_vectors.clamp(-1, 1).detach().cpu().numpy().tolist())
            ...
            ax1.set_title('Real label', fontsize=10)

            ax2 = fig.add_subplot(1, 2, 2)
            ax2.set_xlabel('Principal Component 1', fontsize=7)
            ax2.set_ylabel('Principal Component 2', fontsize=7)
            ax2.set_title('Predict label', fontsize=10)

            colors = ["#ff0000", "#ffaa00", "#fffb00", "#95ff00", "#058505",
                      "#27e8d2", "#d6efff", "#0008ff", "#9900ff", "#ff00ee"]

            real_labels = np.array(real_labels)
            predict_labels = np.array(predict_labels)
            coordinates_2d = np.array(coordinates_2d)

            data_coordinates = coordinates_2d[:-10]
            label_coordinates = coordinates_2d[-10:]

            for i, color in enumerate(colors):
                r_idx = real_labels == i
                p_idx = predict_labels == i

                ax1.scatter(
                    data_coordinates[r_idx][:, 0],
                    data_coordinates[r_idx][:, 1],
                    c=color,
                    s=10,
                )
                ax2.scatter(
                    data_coordinates[p_idx][:, 0],
                    data_coordinates[p_idx][:, 1],
                    c=color,
                    s=10,
                )
            ...
            for i, color in enumerate(colors):
                ax2.scatter(
                    label_coordinates[i, 0],
                    label_coordinates[i, 1],
                    marker="X",
                    edgecolors="#000000",
                    c=color,
                    s=160,
